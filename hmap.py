...
source=ColumnDataSource(pd.read_csv('./iris.csv'))
X=source.to_df()
print(X.columns)
#should be sepal_length, sepal_width, petal_length, petal_width and species

def map_targets_to_numbers():
    global X
    targets=list(set(X['species']))
    t=X['species']
    mapper_dict=dict(list(zip(targets,range(len(targets)))))
    X['target'] = list(map(lambda x:mapper_dict[x],t))

map_targets_to_numbers()
